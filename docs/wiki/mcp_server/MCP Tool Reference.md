...
系统共暴露 **39 个工具**，按功能划分为四大类别。所有工具均通过 FastMCP 框架的 `@mcp.tool()` 装饰器注册，自动获得 JSON-RPC 协议兼容性、参数校验和错误响应处理。

### 1. 查询工具

查询工具提供对决策记忆的只读访问，涵盖列表浏览、议题筛选、全文搜索、关系网络、热度分析和决策树查询等场景。共 **18 个工具**。
...
| `recent_decisions` | 获取最近创建的决策（按小时筛选） |
| `show_tree` | 展示完整决策森林（所有根决策及其递归后代，无需参数） |
