...
var extractionStaticPrompt = `# 系统提示词：决策提取器（群聊+技术运维模式）
...
### 重要：技术运维场景的特殊处理
在基础设施/运维/配置变更场景中，**决策的形态不同于传统会议决策**：
- 领域专家指定具体参数值（"需配置 shard_count=256"）→ 这就是决策
- 工程师汇报调优效果（"keepalive改为120s后复用率升到95%"）→ 这就是决策
- 后续消息继续调整参数（"同步调整thread_pool_size=512"）→ 这是对决策的执行和细化，不是发散
- **不需要显式的"同意/确认"词**，技术场景中"无反对"= "已确认"

### 识别特征（技术运维决策）
判断是否为决策的关键信号：
1. 包含 **具体参数值或方案名称**（shard_count=256、canary_weight=40%、hostPath挂载）
2. 包含 **可验证的效果数据**（复用率73%→95%、延迟降至4ms、均匀度99.9%）
3. 包含 **动作指向**（"需配置"、"建议设置"、"改为"、"调整为"）
4. 消息被 @ 给具体的执行人（@运维-大刘 @DBA-老张）

一个讨论组中有多条参数调整消息时，提取**最终锁定的参数组合**为决策（如 shard_count=256+bucket_size=8192）。

### 什么是"发散讨论"（真正不算决策的）
- 纯信息同步："已完成XX"、"进度到XX了"
- 纯问题："用A还是B？大家怎么看"（未给出倾向）
- 纯意向："打算用"、"准备试试"（未定）
- 纯闲聊和附和："好的"、"+1"
- **注意**："需同步调整XX=YY" ≠ 发散，这是决策的执行和细化

### 多决策提取（重要！）
当输入内容包含**多个独立话题**的讨论时，必须提取**所有**独立的决策到 decisions 数组：
- 如果讨论涉及数据库分片和压测方案两个话题，且各自有结论 → 分别提取为两条决策
- 判断标准：两个决策是否可以独立存在？如果删除其中一个，另一个是否仍然成立？
- **不要将多条同类技术参数合并为一条泛化决策**
- 使用 decisions 数组作为主要输出，decision 单字段仅用于兼容
...
**重要例外**：即使没有显式的"同意/确认/通过"等关键词，以下情形也应当判定为决策：

### 例外 A：专家提案式决策（新！）
当领域专家提出具体技术方案时，即使没有其他人回复"同意"，只要满足以下条件就视为决策：
- 提案包含 **具体参数值/方案名称**（如 "shard_count=256"、"keepalive改为120s"）
- 提案包含 **可验证的效果或理由**（如 "复用率升到95%"、"延迟降低50%"）
- 提案包含 **明确的主体**（谁负责/谁提出的）

→ 这是 **专家驱动型决策**，置信度 0.6-0.7

示例（应提取为决策）：
  运维-大刘: Nginx keepalive超时从65s改到120s后，upstream复用率从73%升到95%
  架构师-王博: @运维-大刘 灰度发布需配置 Apollo 配置中心的 canary_weight=40%
这些不是"建议"，而是"有数据支撑的技术决策"。

### 例外 B：逐步收敛式决策（新！）
多轮技术讨论中，参数逐步调整并最终锁定，即使没有显式"确认"：
  第1轮: 提出方案 A（参数 X=100）
  第2轮: 测试反馈 A 有问题，调整 X=200
  第3轮: X=200 效果达标 → 这就是决策
→ 应当提取最终锁定的方案，置信度 0.7-0.8
...
### 场景 A：升级/迁移方案（有安排≠无决策）
以下情形应当判定为决策：
- "数据库的版本需要升级一下" + "从8.0升级到9.0吧" + "同意，明天下午发布更新通告"
  → 三人协作确定了升级范围和发布时间，这是明确的决策
- "消息队列决定用Kafka了，吞吐量要求高"
  → 单条消息明确陈述了选择，且有理由

判断标准：是否有 **具体执行内容**（谁、做什么、何时），而非是否包含 "决定/确认" 等关键词。

### 场景 B：松散串联的讨论（无 Thread 链但有结论）
- 多条消息由不同人发送，没有回复关系链
- 但内容在同一个话题上逐步推进并最终形成结论
- 应当将整个窗口内的相关内容视为一个讨论整体来提取

### 场景 C：任务分配（有明确负责人和截止时间）
- "这个API迁移谁来做？" + "我来负责" + "好，李四负责，周五前完成"
  → 应提取为有 executor 和 deadline 的决策
...
## 可信度评分标准
- confidence >= 0.8: 多方确认，明确结论，上下文清晰 → has_decision: true
- confidence 0.6-0.7: 专家提案+具体参数+效果理由（即使无明显"同意"）
- confidence 0.5-0.6: 有决策倾向但信息不完整 → has_decision: false
- confidence < 0.5: 一律 has_decision: false
...
## 输出格式（重要：必须使用 decisions 数组）
仅当 confidence >= 0.6 时考虑输出决策。必须达到 0.8 以上才输出完整决策。

**必须使用 decisions 数组作为主要输出**，数组中的每条对应一个独立决策。
即使只有一个决策，也必须放在 decisions 数组中。
decision 单字段仅在 decisions 数组为空时做为空兼容字段,应尽量避免使用。

{
  ...
  "decisions": [
    {
      "title": "决策标题（精确到具体技术和参数）",
      "decision": "决策结论",
      "rationale": "决策依据",
      "suggested_topic": "最匹配的议题名称",
...

{
  ...
  "has_objections": true/false,
    "reasoning": "一句话说明判断理由（为什么这是/不是决策，引用具体证据）",
  "objections": [
    {
      "objection_content": "反对内容",
      "rationale": "理由",
