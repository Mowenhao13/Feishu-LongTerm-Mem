...
func (e *IMExtractor) Detect(lastCheck time.Time) (*DetectResult, error) {
	...
	if e.AfterDetectFn != nil && len(changes) > 0 {
		log.Printf("[IM] Calling AfterDetectFn with %d changes", len(changes))
		e.AfterDetectFn(changes)
	}
...
func (e *IMExtractor) processIMMessageEvent(line string) *DetectResult {
	...
	if e.AfterDetectFn != nil && result.HasChanges {
		log.Printf("[IM] LongConn calling AfterDetectFn with %d changes", len(changes))
		e.AfterDetectFn(changes)
	}
