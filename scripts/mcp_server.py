"""
mcp_server.py — MCP (Model Context Protocol) 服务器

通过 stdio 传输协议暴露决策记忆查询工具给 OpenClaw、Claude Desktop 等 MCP 客户端。

参考: ref/llm/mcp/server/server.go (Go 版 MCP Server)
...
import signal as _builtin_signal  # noqa: E401
...
提供以下工具:
1. list_decisions — 列出所有决策（无需参数，返回全部）
2. topic — 按议题查询决策（省略 topic_id 则返回全部）
3. search — 搜索决策（优先语义检索，降级关键词）
4. decision — 获取单个决策详情
5. timeline — 决策时间线
...
23. extract_decision — 从文本提取决策
24. classify_topic — 议题归类
25. detect_crosstopic — 检测跨议题影响
26. check_conflict — 检测冲突
27. evaluate_dedup — 去重/冲突评估
28. resolve_conflict_action — 获取冲突解决建议
29. revert_decision — 回滚决策
30. confirm_decision — 确认决策
31. reject_decision — 拒绝决策
32. resolve_conflict — 解决冲突
...
class MemoryLoader:
    ...
    def ensure_loaded(self) -> None:
        ...
        graph = MemoryGraph()
        topics = self._storage.list_topics(PROJECT)
        for topic in topics:
            decs = self._storage.list_decisions(PROJECT, topic)
            for dec in decs:
...
def _node_to_dict(node: Any) -> Dict[str, Any]:
    return {
        ...
        "version": getattr(node, "version", "v1.0"),
...
@mcp.tool(
    name="decision",
    description="通过 SDRID 获取单个决策的详细信息。",
)
    ...
    for d in _loader.decisions:
        if hasattr(d, "relations") and d.relations:
            for r in d.relations:
                if hasattr(r, "type") and (r.type.value == "CONFLICTS_WITH" if hasattr(r.type, "value") else str(r.type) == "CONFLICTS_WITH"):
