...
- Decision: Decision extracted from topic discussions
"""
...
from src.utils.datetime_utils import to_iso_format
...
class Decision:
    """
    Decision - A conclusive choice, plan, or action item extracted from topic discussions.
    """
    decision_id: str
    title: str
    content: str
    ...
    source_topic_ids: Optional[List[str]] = None
    rationale: str = ""
    ...
    def __post_init__(self):
        if self.source_topic_ids is None:
            self.source_topic_ids = []

    def __repr__(self) -> str:
        return f"Decision(id={self.decision_id}, title='{self.title}', status={self.status.value})"
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "created_at": to_iso_format(self.created_at) if self.created_at else None,
            "updated_at": to_iso_format(self.updated_at) if self.updated_at else None,
        }